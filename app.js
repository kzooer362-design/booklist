...
class BookListApp {
    ...
    async searchBooks() {
        ...
        if (this.backendApiEnabled && this.backendApiUrl) {
            searchPromises.push({
                name: 'backend',
                promise: this.fetchWithTimeout(`${this.backendApiUrl}/api/search?q=${encodeURIComponent(query)}&fast=1`, {
    ...
    renderBookCard(book, showAddBtn = true) {
        ...
        // 简介：有则显示，无则显示"加载简介"按钮（懒加载）
        let snippetHtml = '';
        if (book.snippet) {
            snippetHtml = `<div class="result-snippet">${this.escapeHtml(book.snippet.substring(0, 100))}${book.snippet.length > 100 ? '...' : ''}</div>`;
        } else if (book._descLoading) {
            snippetHtml = `<div class="result-snippet" style="color:#a0aec0;">加载中...</div>`;
        } else {
            snippetHtml = `<div class="result-snippet" style="cursor:pointer;color:#667eea;" onclick="event.stopPropagation();app.loadBookDetail('${key}')">📖 点击加载简介</div>`;
        }
    ...
    async loadBookDetail(key) {
        ...
        if (!book) {
            ...
            // 同时检查是否在已选列表中（对象引用相同）
            inSelected = this.selectedBooks.includes(book);
        }
        if (!book || book.snippet) return;

        // 标记加载中
        book._descLoading = true;
        this.renderSearchResults();
        if (inSelected) this.renderSelectedBooks();

        try {
            const apiUrl = this.backendApiUrl 
                ? `${this.backendApiUrl}/api/book/detail?q=${encodeURIComponent(book.title)}`
                : `/api/book/detail?q=${encodeURIComponent(book.title)}`;
            const resp = await this.fetchWithTimeout(apiUrl, {}, 10000);
            const data = await resp.json();
            if (data.success && data.description) {
                book.snippet = data.description;
                book._descLoading = false;
                // 如果作者为空也补充
                ...
                this.renderSearchResults();
                if (inSelected) this.renderSelectedBooks();
                this.showToast(`已加载《${book.title}》简介`);
            } else {
                book._descLoading = false;
                book.snippet = '暂无简介';
                this.renderSearchResults();
            ...
            console.warn('懒加载简介失败:', e);
            book._descLoading = false;
            this.renderSearchResults();
            if (inSelected) this.renderSelectedBooks();
            this.showToast('简介加载失败');
        }
    }

    // 一键加载所有缺简介的书
    async loadAllDescriptions() {
        const needDesc = this.selectedBooks.filter(b => !b.snippet);
        if (needDesc.length === 0) {
            this.showToast('所有书籍已有简介');
            return;
        }

        // 检查是否正在加载
        if (needDesc.some(b => b._descLoading)) {
            this.showToast('正在加载中，请稍候...');
            return;
        }

        this.showToast(`正在加载 ${needDesc.length} 本书的简介...`);

        // 标记加载中
        needDesc.forEach(b => b._descLoading = true);
        this.renderSelectedBooks();

        // 并行加载所有简介（限制并发数避免服务器压力）
        const batchSize = 3;
        let loaded = 0;
        for (let i = 0; i < needDesc.length; i += batchSize) {
            const batch = needDesc.slice(i, i + batchSize);
            const results = await Promise.allSettled(
                batch.map(book => this._fetchSingleDescription(book))
            );
            // 更新进度
        ...
        const successCount = needDesc.filter(b => b.snippet && b.snippet !== '暂无简介').length;
        this.showToast(`已加载 ${successCount}/${needDesc.length} 本书的简介`);
    }

    // 单本书简介获取（供批量调用）
    async _fetchSingleDescription(book) {
        try {
            const apiUrl = this.backendApiUrl 
                ? `${this.backendApiUrl}/api/book/detail?q=${encodeURIComponent(book.title)}`
                : `/api/book/detail?q=${encodeURIComponent(book.title)}`;
            ...
            console.warn(`加载《${book.title}》简介失败:`, e);
            book.snippet = '暂无简介';
            book._descLoading = false;
            return false;
        }
    ...
    renderSelectedBooks() {
        ...
        // 统计缺简介的书
        const needDesc = this.selectedBooks.filter(b => !b.snippet);
        const loadingDesc = this.selectedBooks.filter(b => b._descLoading);

        // 顶部工具栏：一键加载简介
        let toolbarHtml = '';
        if (needDesc.length > 0) {
            const btnText = loadingDesc.length > 0 
                ? `⏳ 加载中... (${loadingDesc.length}/${needDesc.length})`
                : `📖 一键加载全部简介 (${needDesc.length}本缺简介)`;
            const disabled = loadingDesc.length > 0 ? 'disabled' : '';
            toolbarHtml = `<div style="margin-bottom:10px;padding:8px 12px;background:#f0f4ff;border-radius:8px;display:flex;align-items:center;justify-content:space-between;">
                <span style="font-size:0.85rem;color:#4a5568;">${needDesc.length}本书暂无简介</span>
                <button class="btn" style="padding:4px 12px;font-size:0.85rem;" ${disabled} onclick="app.loadAllDescriptions()">${btnText}</button>
            </div>`;
        }

        container.innerHTML = toolbarHtml + this.selectedBooks.map(book => {
            // 简介：有则显示，无则显示"加载简介"按钮（懒加载）
            let snippetHtml = '';
            if (book.snippet) {
                snippetHtml = `<div class="result-snippet" style="cursor:pointer;" onclick="event.stopPropagation();app.showSelectedBookDescription('${book.key}')">${this.escapeHtml(book.snippet.substring(0, 80))}${book.snippet.length > 80 ? '...' : ''}</div>`;
            } else if (book._descLoading) {
                snippetHtml = `<div class="result-snippet" style="color:#a0aec0;">⏳ 加载中...</div>`;
            } else {
                snippetHtml = `<div class="result-snippet" style="cursor:pointer;color:#667eea;" onclick="event.stopPropagation();app.loadBookDetail('${book.key}')">📖 点击加载简介</div>`;
            }
    ...
    showResult(listId) {
        ...
        ` + sortedBooks.map(book => {
            ...
            const zlibraryUrl = `https://zh.z-library.im/s/${encodeURIComponent(book.title)}`;
