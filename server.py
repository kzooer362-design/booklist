...
- 图片源（cover）: 豆瓣CDN（doubanio.com，为缺封面书兜底）
- 各字段击中一个源即可使用，不必强求一个源给全所有信息
- 单次请求返回尽可能多的高质量结果

快速模式（默认 fast=1）: 仅调豆瓣 suggest API，秒级返回 title/author/cover/year
完整模式（fast=0）: 补充详情页抓取 + 简介/封面兜底
懒加载: /api/book/detail 按需获取简介
"""
...
def search_douban_cover(query):
    """豆瓣CDN封面搜索 - 通过豆瓣suggest API获取书籍封面（doubanio.com CDN）"""
    print(f'[豆瓣CDN] 搜索封面: {query}')
    try:
        ...
        for item in data:
            ...
            cover = item.get('pic', '')
            if cover:
                # 升级为大图：/s/public/ → /l/public/
                if '/s/public/' in cover:
...
def fill_missing_covers(books, max_fill=8):
    ...
    # 限制数量，避免过多请求
    missing = missing[:max_fill]
    print(f'[封面兜底] 为 {len(missing)} 本缺封面/无效封面的书并发搜图（豆瓣CDN）')
    ...
    def fill_one(idx_book):
        ...
        if not title or len(title) < 1:
            return idx, None
        # 豆瓣CDN：通过豆瓣suggest API获取封面（doubanio.com）
        cover = search_douban_cover(title)
        if cover:
            return idx, cover
        return idx, None
...
def search_douban_suggest(query, fast=True):
    """豆瓣 suggest API + 搜索页 - 主搜索源

    fast=True（默认）: 仅调 suggest API，秒级返回 title/author/cover/year，不抓详情页
    fast=False: 完整模式，补充搜索页结果 + 并发抓取详情页简介
    """
    print(f'[豆瓣Suggest] 搜索: {query} (fast={fast})')
    books = []
    seen_ids = set()
    ...
    try:
        ...
        # fast 模式直接返回，不抓详情页
        if fast:
            print(f'[豆瓣Suggest] fast模式，直接返回 {len(books)} 本')
            return books

        # ===== 完整模式：仅尝试为前3本书抓取简介（3秒超时，避免拖慢）=====
        need_desc = [b for b in books if not b.get('description')][:3]
        if need_desc:
            print(f'[豆瓣Suggest] 尝试为 {len(need_desc)} 本抓取简介（3s超时）')
            ...
            def fetch_desc(book):
                ...
                try:
                    ...
                    print(f'[豆瓣Suggest] 详情页抓取超时/失败: {e}')

            with ThreadPoolExecutor(max_workers=min(3, len(need_desc))) as executor:
...
def search_books():
    ...
    2. 图片源（cover）: 豆瓣CDN（doubanio.com，为缺封面的书兜底补充）
    各字段击中一个源即可使用，不必强求一个源给全所有信息。
    """
    query = request.args.get('q', '').strip()
    # 中国版：默认豆瓣读书优先（国内服务器可正常访问豆瓣）
    sources = [s.strip() for s in request.args.get('sources', 'douban_suggest').split(',')]
    # fast 模式（默认开启）：仅用 suggest API 秒级返回，跳过详情页抓取和兜底补充
    fast = request.args.get('fast', '1').strip() != '0'

    if not query:
        return jsonify({'success': False, 'error': '请输入书名'}), 400

    cache_key = f'{query}_{"|".join(sorted(sources))}_fast{fast}'
    cached = load_cache(cache_key)
    if cached is not None:
        print(f'[缓存] 返回 {len(cached)} 本书')
        return jsonify({'success': True, 'source': 'cache', 'books': cached, 'total': len(cached), 'from_cache': True, 'fast': fast})

    print(f'[搜索] 查询: {query}, 来源: {sources}, fast: {fast}')
    ...
    if 'douban_search_page' in sources or 'all' in sources or 'douban' in sources:
        search_tasks.append(('douban_search_page', lambda q: search_douban_search_page(q)))
    # ===== 兜底源（仅当 sources=all/指定时启用） =====
    if 'douban' in sources or 'all' in sources or 'douban_suggest' in sources:
        search_tasks.append(('douban_suggest', lambda q: search_douban_suggest(q, fast=fast)))
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        try:
            overall_timeout = 30 if not fast else 10
...
@app.route('/api/book/detail', methods=['GET'])
def book_detail():
    """懒加载书籍详情 - 用户点击时按需获取简介/作者

    通过豆瓣 suggest API 找到匹配书，访问详情页抓取简介和作者。
    适用于 fast 模式下前端按需加载。
    """
    ...
    try:
        # 1. 调 suggest API 找到匹配的书
        api_url = 'https://book.douban.com/j/subject_suggest'
        resp = requests.get(api_url, params={'q': title}, headers=headers, timeout=5)
        if resp.status_code != 200:
            return jsonify({'success': False, 'error': 'suggest API 不可用'}), 502

        data = resp.json()
        if not data:
            return jsonify({'success': False, 'error': '未找到匹配书籍'}), 404
        ...
        # 2. 获取详情页 URL
        detail_url = best_match.get('url', '')
        author = best_match.get('author_name', '')
        cover = best_match.get('pic', '')
        if cover and '/s/public/' in cover:
            cover = cover.replace('/s/public/', '/l/public/')
        if cover and cover.startswith('//'):
            cover = 'https:' + cover
        ...
        if detail_url:
            try:
                ...
                print(f'[详情懒加载] 详情页抓取失败: {e}')

        print(f'[详情懒加载] 成功: author={author}, desc_len={len(description)}')
        return jsonify({
            'success': True,
...
def health_check():
    ...
    return jsonify({
        ...
        'image_sources': ['douban_cdn'],
        'endpoints': ['/api/search', '/api/book/detail', '/api/cover', '/api/health', '/api/cache/clear'],
...
if __name__ == '__main__':
    ...
    print('')
    print('📖 接口说明:')
    print('   主接口:  GET /api/search?q=书名 [&sources=douban_suggest|all] [&fast=1|0]')
    print('   详情懒加载: GET /api/book/detail?q=书名')
    ...
    print('🖼️ 图片源: 豆瓣CDN（doubanio.com，为缺封面书兜底）')
    print('⚡ 策略:   分治聚合 - 各字段击中一个源即可使用')
    print('🚀 默认 fast 模式: 秒级返回，适合快速建立投票')
    print('')
    print('💡 示例:')
    print('   http://localhost:5000/api/search?q=三体')
    print('   http://localhost:5000/api/search?q=三体&fast=0  (完整模式)')
    print('   http://localhost:5000/api/book/detail?q=三体    (懒加载简介)')
